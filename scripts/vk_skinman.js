...
function vkgetCookie(name,temp){ //this func is duplicate from vkopt.js
...
function vkLoadCss(c,notid) {
    var headID = document.getElementsByTagName("head")[0];
    var cssNode = document.createElement('link');
    cssNode.type = 'text/css';
    cssNode.rel = 'stylesheet';
    if (!notid){
      cssNode.id="vkStyleCSS";
    }
    cssNode.href = c;
...
function vkSetCSSCode(){
  ...
  if (!cur_css) cur_css="";
  if (!window.vkCSSCodeBox) vkCSSCodeBox = new MessageBox({title: IDL('CSSCode'),closeButton:true,width:"500px"});
  vkCSSCodeBox.removeButtons();
  vkCSSCodeBox.addButton({
    onClick: function(){ vkCSSCodeBox.hide(200);},
    style:'button_no',label:IDL('Hide')
  });
  vkCSSCodeBox.addButton({
    onClick: function(){ ge('vkcsscode').value=""; ge('vkcsscode').focus(); },
    style:'button_no',label:IDL('Clear')
  });
  vkCSSCodeBox.addButton({
    onClick: function(){ 
...
function vkShowSkinMan(filter,page){
  if (!window.VK_STYLE_LIST){
      ge("content").innerHTML='<div class="box_loader"></div>';
      var nows= new  Date(); 
  ...
  for (var i=from; i<to;i++){
     ...
     html +=""+
          '<div class="'+(vkGetVal("VK_CURRENT_CSS_URL")==CssUrl?'current_skin':'noselected_skin')+'">'+
            '<div><h6 onclick="return vkSwichStyle(\''+CssUrl+'\',this);" style="cursor:hand;">'+Name+'</h6></div>'+
...
function vkSkinManInit(){
  ...
  if (getSet(81)=='n' || location.href.match(/widget_.+php/) || vkbrowser.mozilla) return;
