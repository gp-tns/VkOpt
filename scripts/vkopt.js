...
function InstallRelease(){
  ...
  if (!window.vkMsg_Box || isNewLib()) vkMsg_Box = new MessageBox({title: IDL('THFI'),width:"495px"});
...
function new_check(wall) {
  ...
  var items=[];
  for (var i=0;i<nodes.length;i++)
    if (nodes[i].parentNode.id=='nav') items.push(nodes[i]);
...
function vkMenu(){//vkExLeftMenu
  ...
  for (var i=0;i<items.length;i++) if (items[i].parentNode.tagName=='LI'){
    ...
    page=(page)?page[1]:(item.className=='hasedit')?'profile':'';
